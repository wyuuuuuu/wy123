...
SOURCE_CHUNKS_PATH = ROOT_DIR / "data" / "source_text_chunks.json"
SOURCE_CHUNKS_REVISED_PATH = ROOT_DIR / "data" / "source_text_chunks_display_revised_fixed_with_supplements.json"
...
def source_chunk_candidates() -> list[Path]:
    """Return possible source-text index locations for local and cloud deployment."""
    candidates = [
        SOURCE_CHUNKS_REVISED_PATH,
        ROOT_DIR / "source_text_chunks_display_revised_fixed_with_supplements.json",
        SOURCE_CHUNKS_PATH,
        ROOT_DIR / "data" / "intermediate" / "chunks.json",
    ]
    for parent in ROOT_DIR.parents:
        candidates.append(parent / "data" / "intermediate" / "chunks.json")
    unique: list[Path] = []
...
@st.cache_data(show_spinner=False)
def load_source_chunks() -> dict[str, dict]:
    """Load the original textbook text chunks used by the extraction pipeline."""
    for path in source_chunk_candidates():
        if not path.exists():
            continue
        try:
        ...
        for record in records:
            if not isinstance(record, dict):
                continue
            chunk_id = str(record.get("chunk_id", "") or record.get("block_id", "")).strip()
            if chunk_id:
...
def normalize_evidence_text(value: object) -> str:
    """Clean evidence text before showing it as a short textbook excerpt."""
...
def excerpt_around_keyword(text: object, keyword: str, char_limit: int = 50) -> str:
    """Extract about 50 characters around a knowledge point and wrap it with ellipses."""
    ...
    if keyword:
        ...
        if keyword_index < 0:
            keyword_index = cleaned.lower().find(keyword.lower())

    if keyword_index < 0:
        snippet = cleaned[:char_limit]
    else:
        target_len = max(char_limit, len(keyword) + 12)
        left_context = max(6, (target_len - len(keyword)) // 2)
        start = max(0, keyword_index - left_context)
...
def edge_relation_text(row: pd.Series | dict) -> str:
    """Format one edge as a user-facing relation description."""
    return f"{row.get('source_name', '')} → {display_relation(row.get('relation', ''))} → {row.get('target_name', '')}"


def source_text_from_chunk(record: dict, fallback_text: object = "") -> str:
    """Combine the source chunk and nearby context into a readable textbook paragraph."""
    if not record:
        return normalize_evidence_text(fallback_text)
    revised = normalize_evidence_text(
        record.get("display_text_ai_revised", "")
        or record.get("display_text_revised", "")
        or record.get("display_text", "")
    )
    if revised:
        return revised
    before = normalize_evidence_text(record.get("context_before", ""))
    main = normalize_evidence_text(record.get("raw_text", "") or record.get("clean_text", "") or fallback_text)
    after = normalize_evidence_text(record.get("context_after", ""))
    parts = [part for part in [before, main, after] if part]
    return "\n\n".join(parts) if parts else normalize_evidence_text(fallback_text)


def display_text_for_chunk(record: dict) -> str:
    """Return the best display text from a source chunk record."""
...
def find_chunk_text_by_node_name(node_name: str) -> dict:
    """Find a revised textbook chunk whose display text directly contains the node name."""
    name = normalize_evidence_text(node_name)
    if not name:
        return {}

    exact_matches: list[dict] = []
    relaxed_matches: list[dict] = []
    relaxed_name = re.sub(r"\s+", "", name).lower()
    for record in load_source_chunks().values():
        display_text = display_text_for_chunk(record)
        if not display_text:
            continue
        if name in display_text:
            exact_matches.append(record)
            continue
        if relaxed_name and relaxed_name in re.sub(r"\s+", "", display_text).lower():
    ...
    def score(record: dict) -> tuple[int, int]:
        text = display_text_for_chunk(record)
        return (text.find(name) if name in text else 9999, len(text))

    best = min(matches, key=score)
    return {
        "来源文件": best.get("source_file", "") or "暂无来源文件",
        "页码": str(best.get("page_num", "") or best.get("page_no", "") or "暂无页码"),
        "片段编号": best.get("chunk_id", "") or best.get("block_id", "") or "暂无片段编号",
        "对应关系": f"包含知识点：{name}",
        "原文片段": display_text_for_chunk(best),
        "教材展示文本": display_text_for_chunk(best),
        "原始教材文本": normalize_evidence_text(best.get("raw_text", "")),
        "原始文件路径": str(resolve_raw_doc_path(best.get("source_file", "")) or ""),
    }


def raw_source_text_from_chunk(record: dict, fallback_text: object = "") -> str:
    """Return unpolished source text for optional traceability display."""
    if not record:
        return normalize_evidence_text(fallback_text)
    before = normalize_evidence_text(record.get("context_before", ""))
    main = normalize_evidence_text(record.get("raw_text", "") or record.get("clean_text", "") or fallback_text)
...
def raw_doc_roots() -> list[Path]:
    """Return possible raw textbook directories for local and cloud packages."""
    candidates = [RAW_DOCS_DIR]
    for parent in ROOT_DIR.parents:
        candidates.append(parent / "data" / "raw_docs")
    unique: list[Path] = []
    for path in candidates:
        if path not in unique:
            unique.append(path)
...
def resolve_raw_doc_path(source_file: object) -> Path | None:
    """Find the original PDF/docx file if it is available in the package."""
    source_name = Path(str(source_file or "")).name
    if not source_name or source_name == ".":
        return None

    for root in raw_doc_roots():
        if not root.exists():
            continue
        exact_path = root / source_name
...
def source_record_for_edge(row: pd.Series | dict, keyword: str = "") -> dict:
    """Resolve one graph edge back to the original textbook text block when possible."""
    chunk_id = str(row.get("block_id", "") or row.get("chunk_id", "")).strip()
    chunk = load_source_chunks().get(chunk_id, {}) if chunk_id else {}
    source_file = chunk.get("source_file") or row.get("source_file", "") or "暂无来源文件"
    page_num = chunk.get("page_num", "") or chunk.get("page_no", "")

    reviewed_text = normalize_evidence_text(row.get("evidence_text", ""))
    matched_record = find_chunk_text_by_node_name(keyword or row.get("source_name", "") or row.get("target_name", ""))
    if matched_record:
        return matched_record

    # Fall back to the final triples' exact evidence sentence. Chunk ids from
    # different intermediate files can collide, so only use chunk text if it
    # visibly contains the relation terms.
    raw_chunk_text = raw_source_text_from_chunk(chunk, "")
    revised_chunk_text = source_text_from_chunk(chunk, "")
    relation_terms = [
        str(keyword or "").strip(),
        str(row.get("source_name", "") or "").strip(),
        str(row.get("target_name", "") or "").strip(),
    ]
    chunk_text = revised_chunk_text or raw_chunk_text
    chunk_matches_relation = bool(chunk_text) and any(term and term in chunk_text for term in relation_terms)
    display_text = reviewed_text or (chunk_text if chunk_matches_relation else "")
    raw_trace_text = raw_chunk_text if chunk_matches_relation else ""
    return {
        "来源文件": str(source_file),
        "页码": str(page_num) if page_num not in {None, ""} else "暂无页码",
        "片段编号": chunk_id or "暂无片段编号",
        "对应关系": edge_relation_text(row),
        "原文片段": display_text,
        "教材展示文本": display_text,
        "原始教材文本": raw_trace_text,
        "原始文件路径": str(resolve_raw_doc_path(source_file) or ""),
    }


def node_source_records(node_id: str, node_name: str, edges: pd.DataFrame, limit: int = 3) -> list[dict]:
    """Collect full source-text records related to one selected knowledge point."""
    if edges.empty:
        return []

    related = edges[(edges["source"] == node_id) | (edges["target"] == node_id)].copy()
    if related.empty:
        return []
    related = related[related["relation"].isin(RELATION_OPTIONS)].copy()
    if related.empty:
        return []

    records: list[dict] = []
    seen: set[tuple[str, str, str]] = set()
    matched_record = find_chunk_text_by_node_name(node_name)
    if matched_record:
        seen.add((
            matched_record["来源文件"],
            matched_record["片段编号"],
            matched_record.get("教材展示文本", ""),
        ))
        records.append(matched_record)

    for _, row in related.iterrows():
        record = source_record_for_edge(row, keyword=node_name)
        if not record.get("教材展示文本") and not record.get("原始教材文本"):
            continue
        dedup_key = (record["来源文件"], record["片段编号"], record.get("教材展示文本") or record.get("原始教材文本", ""))
...
def node_evidence_records(node_id: str, node_name: str, edges: pd.DataFrame, limit: int = 3) -> list[dict]:
    """Collect short, traceable textbook snippets related to one knowledge point."""
    records = node_source_records(node_id, node_name, edges, limit=limit)
    return [
        {
            "原文片段": item.get("原文片段", ""),
            "来源文件": item.get("来源文件", ""),
...
def node_evidence_summary(node_id: str, node_name: str, edges: pd.DataFrame, limit: int = 2) -> str:
    """Build a compact evidence summary for the graph-side detail panel."""
    records = node_evidence_records(node_id, node_name, edges, limit=limit)
    if not records:
        return "暂无直接教材片段"
    return "；".join(
        f"{item['原文片段']}（{item['来源文件']} {item['片段编号']}）"
        for item in records
    )


def render_source_text_records(records: list[dict], *, title: str = "教材证据句") -> None:
    """Render source-text blocks as expandable, readable textbook evidence."""
    if not records:
        st.info("当前知识点暂无可直接展示的教材证据。")
        return

    st.markdown(f"**{title}**")
    st.caption("默认展示该知识点直接相连三元组的教材证据句，不额外拼接前后文。")
    for index, record in enumerate(records, start=1):
        source_file = record.get("来源文件", "暂无来源文件")
        block_id = record.get("片段编号", "暂无片段编号")
        page_num = record.get("页码", "暂无页码")
        relation_text = record.get("对应关系", "暂无对应关系")
        expander_title = f"{index}. {source_file}｜{block_id}"
        with st.expander(expander_title, expanded=True):
            st.markdown(
                f'<div class="kg-source-meta">页码：{html.escape(str(page_num))}　对应关系：{html.escape(str(relation_text))}</div>',
                unsafe_allow_html=True,
            )
            st.markdown(
                f'<div class="kg-source-text">{html.escape(str(record.get("教材展示文本", "") or record.get("原文片段", "")))}</div>',
                unsafe_allow_html=True,
            )
            raw_doc_path = Path(str(record.get("原始文件路径", "") or ""))
            if raw_doc_path.exists() and raw_doc_path.is_file():
                file_size_mb = raw_doc_path.stat().st_size / (1024 * 1024)
                if file_size_mb <= 30:
                    mime_type = mimetypes.guess_type(raw_doc_path.name)[0] or "application/octet-stream"
                    st.download_button(
                        "下载原始教材文件",
                        data=raw_doc_path.read_bytes(),
                        file_name=raw_doc_path.name,
                        mime=mime_type,
                        key=f"download_source_{index}_{block_id}_{raw_doc_path.name}",
                        use_container_width=True,
                    )
                else:
                    st.caption(f"已定位原始文件：{raw_doc_path.name}，但文件约 {file_size_mb:.1f} MB，页面不自动加载大文件。")
    ...
    *,
    ...
    for _, row in nodes.iterrows():
        ...
        if display_role == "overview_center":
            ...
            border_width = 5
            font_size = 19
        source_records = node_source_records(row["node_id"], row["name"], edges, limit=1)
        source_popup_text = source_records[0].get("教材展示文本", "") if source_records else ""
        node_items.append(
            {
                "id": row["node_id"],
                "label": row["name"],
                "sourceSnippet": source_popup_text or "暂无教材片段",
                ...
                "detail": {
                    ...
                    "教材原文片段": node_evidence_summary(row["node_id"], row["name"], edges),
                },
            }
        )
    ...
    for _, row in edges.iterrows():
        ...
        edge_source = source_record_for_edge(row, keyword=row.get("source_name", ""))
        edge_items.append(
            {
                "id": row["edge_id"],
                "from": row["source"],
                ...
                "detail": {
                    ...
                    "教材原文片段": edge_source.get("原文片段", "") or "无",
...
<body>
  <div class="wrap">
    <div class="graph-panel">
      ...
      <div id="network"></div>
      <div id="sourcePopup" class="source-popup">
        <div class="source-popup-head">
          <span>教材片段</span>
          <button id="closeSourcePopup" class="source-popup-close" type="button">×</button>
        </div>
        <div id="sourcePopupText" class="source-popup-text"></div>
      </div>
  ...
  <script>
    ...
    function showSourcePopup(nodeId, point) {
      const popup = document.getElementById("sourcePopup");
      const textBox = document.getElementById("sourcePopupText");
      const panel = document.querySelector(".graph-panel");
      const node = nodes.get(nodeId) || {};
      const detail = nodeDetails[nodeId] || {};
      const text = node.sourceSnippet || detail["原始教材文本"] || detail["教材原文片段"] || "暂无教材片段";
      textBox.textContent = text || "暂无教材片段";
      popup.style.display = "block";
      const popupWidth = Math.min(390, Math.max(280, panel.clientWidth - 32));
      popup.style.width = popupWidth + "px";
      const maxLeft = Math.max(12, panel.clientWidth - popup.offsetWidth - 12);
      const maxTop = Math.max(12, panel.clientHeight - popup.offsetHeight - 12);
      const left = Math.min(Math.max(12, point.x + 12), maxLeft);
      const top = Math.min(Math.max(12, point.y + 12), maxTop);
    ...
    network.on("oncontext", params => {
      ...
      if (nodeId) showSourcePopup(nodeId, params.pointer.DOM);
      else hideSourcePopup();
    });
    network.on("doubleClick", params => {
      if (params.nodes.length) focusNode(params.nodes[0]);
      else if (isFocused) resetGraph();
    });
    document.getElementById("network").addEventListener("contextmenu", event => event.preventDefault());
    document.getElementById("closeSourcePopup").addEventListener("click", hideSourcePopup);
            ...
            node_items.append(
                {
                    ...
                    "detail": {
                        ...
                        "教材原文片段": node_evidence_summary(node_id, row.get("name", ""), edges),
                    },
                }
            )
...
def render_teacher(nodes: pd.DataFrame, edges: pd.DataFrame, topics: dict, max_component_size: int) -> None:
    ...
    if topic_node_ids:
        ...
        render_source_text_records(
            node_source_records(detail_node_id, detail_node["name"], detail_edges, limit=3),
            title="教材证据句",
        )
...
def render_student(nodes: pd.DataFrame, edges: pd.DataFrame, topics: dict) -> None:
    ...
    if student_view == "知识点查询":
        ...
        with cols[0]:
            ...
            render_source_text_records(
                node_source_records(selected["node_id"], selected["name"], related, limit=3),
                title="教材证据句",
            )
        with cols[1]:
            st.markdown("**相关知识推荐**")
        ...
        if st.button("查询", use_container_width=True):
            if not question.strip():
                ...
                if not evidence.empty:
                    ...
                    source_records = [
                        source_record_for_edge(row, keyword=row.get("source_name", ""))
                        for _, row in evidence.head(3).iterrows()
                    ]
                    render_source_text_records(source_records, title="命中教材证据句")
